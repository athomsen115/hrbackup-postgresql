...
def test_users_update(mocker):
    ...
    utilize `usermod` to create a user with the password
    and groups
    """
    mocker.patch('subprocess.call')
...
def test_users_sync(mocker):
    """
    Given a list of user dictionaries, `users.sync(...)` should create missing users,
    remove extra non-system users, and update existing users. A list of existing
